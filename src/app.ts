...
function $(id:'editor'|'export-dialog'|'sponge-dialog'|'bernoulli-dialog'|'surface-dialog'): HTMLDialogElement;
...
function regenerate(candidate:Candidate|null=null,automatic=false){
 ...
 const fail=(message:string)=>{if(id!==job)return;clearTimeout(timeout);worker?.terminate();pending=null;$('preset').value=currentPreset;showError(message);if($('editor').open)$('editor-error').textContent=message;if($('sponge-dialog').open)$('sponge-error').textContent=message;if($('bernoulli-dialog').open)$('bernoulli-error').textContent=message;if($('surface-dialog').open)$('surface-error').textContent=message;generating=false;adaptiveGallery=false;if(sample)updatePointStatus();else $('status').textContent='Generation failed';};
 ...
 worker.onmessage=({data})=>{
  ...
  if(!automatic&&(pending||!sample||sample.ids.length!==data.ids.length)){
   viewer!.autoRotate=viewer!.defaultAutoRotate(adaptiveGallery&&!pending?.custom?Math.min(250_000,data.ids.length):data.ids.length);
  ...
  generating=false;viewer!.adaptiveDefault=adaptiveGallery&&currentPreset!=='custom';sample=data;viewer!.setSample(data,maps.length);$('download-image').disabled=false;recolor();
...
function init(){
 try {
  viewer=new FractalViewer($('canvas-container'),()=>showError('The graphics context was lost. Reload this page to restore the view.'),(count,mapCount)=>{renderedCount=count;renderedMaps=mapCount;updatePointStatus();},(budget,slowed)=>{
   if(!adaptiveGallery||currentPreset==='custom'||generating||pending||document.querySelector('dialog[open]'))return;
   const count=Math.max(50_000,Math.floor(budget/10_000)*10_000);
   if(!slowed&&sample&&count<=sample.ids.length)return;
   if(slowed)adaptiveGallery=false;
