...
export class SurfaceEditor{
 ...
 constructor(private onApply:(maps:MapDefinition[],name:string,surface:SurfaceDefinition)=>void){
  ...
  this.invalidateImport=jsonControls(this.dialog,()=>({version:1,type:'surface',name:this.el<HTMLInputElement>('surface-name').value.trim()||'Custom surface',definition:{phi:this.el<HTMLTextAreaElement>('surface-phi').value,lambda:this.el<HTMLInputElement>('surface-lambda').valueAsNumber,base:this.el<HTMLInputElement>('surface-base').value}}),file=>{
   if(file.type!=='surface')throw Error('Import a Weierstrass-type surface JSON file in this panel.');
   clearTimeout(this.previewTimer);this.open(file.definition,file.name);
  },message=>{this.el('surface-error').textContent=message;});
