...
export function compileExpression(source: string): Expression{
 ...
 function parse(min=0): Expression{
  ...
  if(t==='+'||t==='-'){const v=parse(3);left=t==='-'?(x,y,z)=>-v(x,y,z):v;}
  ...
  while(at<tokens.length){const op=tokens[at],precedence: number|undefined=({'+':1,'-':1,'*':2,'/':2,'^':4,'**':4} as Record<string,number>)[op];if(!precedence||precedence<min)break;at++;const right=parse(precedence+(precedence===4?0:1)),l=left;left=(x,y,z)=>{const a=l(x,y,z),b=right(x,y,z);switch(op){case '+':return a+b;case '-':return a-b;case '*':return a*b;case '/':return a/b;default:return a**b;}};}
  return left;
 }
