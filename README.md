...
The [chaos game](https://en.wikipedia.org/wiki/Chaos_game) repeatedly applies a randomly selected map and plots the resulting points. An `IFS` object owns the maps, validation, and seeded sampling. A Web Worker generates points while Three.js renders them. Up to ten million points can be generated. During camera movement, the viewer starts with a 250,000-point budget and increases detail toward the selected count (up to five million) while frames remain smooth, reducing detail when rendering slows; full detail returns when movement settles and is used for exports.
